...
class MemoryGame:
    ...
    def start_game(self, player_address):
        game_thread = threading.Thread(target=self.game_handler,
                                       args=(player_address,))
        game_thread.start()

    def game_handler(self, player_address):
        ...
        self.send_msg('B' + self.board.display_board_x(), player1_address)
        self.send_msg('B' + self.board.display_board_x(), player2_address)
        self.send_msg('1', player1_address)
        self.send_msg("MOVE", player1_address)
        self.send_msg('0', player2_address)

        while self.players[player1_address].turn or self.players[
            player2_address].turn:
            pass
            ...
            if player1.moves == 1:
                ...
                if self.board.check_matching(self.choice1,
                    ...
                    if self.board.is_empty():
                        self.send_msg('E' + str(player2.score) + ' ' +
                                      str(player1.score), player2_address)
                        self.send_msg('E' + str(player1.score) + ' ' +
                                      str(player2.score), player1_address)
                        self.loop = False
                else:
                    player2.moves = 2
